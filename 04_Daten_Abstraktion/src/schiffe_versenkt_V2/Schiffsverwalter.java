...
public static  int z_Schiffe_versenkt = 0;
...
public Schiffsverwalter (int ss_z_schiffe)
	...
	for (int i=0 ; i<z_schiffe; i++)
		{do 
			{schiff_laenge = (int)(Math.random()*10 )%3 + 1;
			...
			isKollisionfrei = Kollisionskontrolle_schiff(schiff_x_pos, schiff_y_pos, schiff_laenge,schiff_ausrichtung);
		...
		Schiff neuesSchiff = new Schiff(schiff_x_pos, schiff_y_pos, schiff_laenge,schiff_ausrichtung);
		lst_schiffe.add(neuesSchiff);
		}
	}

private  boolean Kollisionskontrolle_schiff(int ss_neueschiff_x_pos,int ss_neuesschiff_y_pos,
...
public  void Schuss_auf_Schiffe_setzen(int x_schuss, int y_schuss)
	...
	for (Schiff i_schiff : lst_schiffe)
		{if (i_schiff.isVersenkt())	
			...
			if(i_schiff.getIs_schiffsteil_mehrmal_getroffen())	
				{last_schuss_ergebnis =4;
				}	
			}
		else
			{i_schiff.Set_Schuss(x_schuss, y_schuss);
			if(i_schiff.getIs_schiffsteil_getroffen()) 		
